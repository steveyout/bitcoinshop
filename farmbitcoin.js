    ...
    con.query("SELECT id FROM account WHERE id=" + id, function (err, result, fields) {
        ...
        if (message.text == start&&result.length===0) {
            ...
            con.query("insert into `account` SET ?", user, function (error, results) {
                ...
                console.log(error)
                ctx.reply('welcome ' + ctx.from.first_name + ' to BTC Shop.\n\n🔹Own shops\n🔹Collect income\n🔹Exchange income for 💰\n🔹Earn real Money in BTC', Markup
                    .keyboard([
...
bot.action('👤Refferal link',ctx=>{
    ctx.editMessageText('https://t.me/Bitcoinshop2bot?start='+ctx.from.id,Extra
        .HTML()
        .markup((m) => m.inlineKeyboard([
        ...
        if (results[0].depoaddre== null) {
            client.getAccount(btc, function (err, account) {
                account.createAddress(null, function (err, address) {
                    ...
                    ctx.replyWithHTML('<b>⚡Top up balance</b>\n\nYou can buy game currency for purchasing shops and (💰) using BTC. To top up your balance, simply send<b> any amount</b> of BTC to this address\n\n<code>' + adress + '</code>\n\nThe BTC will be automatically credited to the balance at the rate of\n' + '<b>0.1 BTC = 100,000 💰</b>\n' + '<b>0.01 BTC = 10,000 💰</b>\n' + '<b>0.001 BTC = 1,000 💰</b>\n' + '<b>0.0001 BTC = 100 💰 </b>etc.')
                    var ide =ctx.from.id
...
bot.hears("Admin",ctx => {
    if (ctx.from.id === admin) {
        ctx.replyWithHTML("Admin control panel", Markup
            .keyboard([
                ["sms", "balance"],
                ["points", "withdraw"],
                ["🏠Menu"]
        ...
        ctx.replyWithHTML("<b>this command is restricted to admins only</b>")
    }

})


const smsscene=new Scene("sms")
smsscene.enter((ctx)=>{
    ctx.replyWithHTML("write down the sms you want to send to all users",Markup
        .keyboard([
            ['🛑cancel']
        ])
        .resize()
...
smsscene.hears('🛑cancel',ctx=> {
    ctx.replyWithHTML("<b>cancelled</b>", Markup
        .keyboard([
            ['🏦Shops', '💵Income'], // Row1 with 2 buttons
            ['💸Balance', '👨‍👧‍👦Refferals'], // Row2 with 2 buttons
            ['⚖️Exchange', '📈Stastistics'],
            ...
            con.query(sql, function (error, results, fields) {
                ...
                client.getAccount(btc, function (err, account) {
                    account.sendMoney({
                        ...
                        con.query(sqla)
                        ctx.replyWithHTML('Your withdrawal of ' + payout + ' BCH is being processed', Markup
                            .keyboard([
                                ['🏦Shops', '💵Income'], // Row1 with 2 buttons
                                ['💸Balance', '👨‍👧‍👦Refferals'], // Row2 with 2 buttons
                                ['⚖️Exchange', '📈Stastistics'],
...
///mass points
const pointsscene=new Scene("points")
pointsscene.enter((ctx)=>{
    ctx.replyWithHTML("write down the payout points you want to send to all users",Markup
        .keyboard([
            ['🛑cancel']
        ])
        .resize()
...
pointsscene.on("message",ctx=>{
    if (isNaN(ctx.message.text)){
        ctx.replyWithHTML("<b>Please provide me with a valid number</b>")


    } else {
        var points=ctx.message.text
        con.query("UPDATE account SET `payoutpoints`=`payoutpoints`+"+points,function (err,res) {

            ctx.replyWithHTML("<b>"+points+"</b> payout points have been awarded to all users", Markup
                .keyboard([
                    ['🏦Shops', '💵Income'], // Row1 with 2 buttons
                    ['💸Balance', '👨‍👧‍👦Refferals'], // Row2 with 2 buttons
                    ['⚖️Exchange', '📈Stastistics'],
...
balancescene.enter((ctx)=>{
    ctx.replyWithHTML("write down the purchase balance you want to send to all users",Markup
        .keyboard([
            ['🛑cancel']
        ])
        .resize()
...
balancescene.on("message",ctx=>{
    if (isNaN(ctx.message.text)){
        ...
        var points=ctx.message.text
        con.query("UPDATE account SET `balance`=`balance`+"+points,function (err,res) {

            ctx.replyWithHTML("<b>"+points+"</b> purchase balance have been awarded to all users", Markup
                .keyboard([
                    ['🏦Shops', '💵Income'], // Row1 with 2 buttons
                    ['💸Balance', '👨‍👧‍👦Refferals'], // Row2 with 2 buttons
                    ['⚖️Exchange', '📈Stastistics'],
...
///mass payout
const payoutscene=new Scene("payout")
payoutscene.enter((ctx)=>{
    ctx.replyWithHTML("write down the payout balance you want to send to all users",Markup
        .keyboard([
            ['🛑cancel']
        ])
        .resize()
...
payoutscene.on("message",ctx=>{
    if (isNaN(ctx.message.text)){
        ...
        var points=ctx.message.text
        con.query("UPDATE account SET `payout`=`payout`+"+points,function (err,res) {

            ctx.replyWithHTML("<b>"+points+"</b> payout balance have been awarded to all users", Markup
                .keyboard([
                    ['🏦Shops', '💵Income'], // Row1 with 2 buttons
                    ['💸Balance', '👨‍👧‍👦Refferals'], // Row2 with 2 buttons
                    ['⚖️Exchange', '📈Stastistics'],
...
//scenes
const stage = new Stage([withdrawscene,greeterScene,smsscene,balancescene,payoutscene,pointsscene], { ttl: 1800 })
bot.use(session())
bot.use(stage.middleware())
bot.action('🔰Withdraw',enter('withdraw'))
bot.hears('🔑set withdraw address', enter('greeter'))
bot.hears('Admin',enter('admin'))
bot.hears('sms',enter('sms'))
bot.hears('points',enter('points'))
bot.hears('withdraw',enter('payout'))
bot.hears('balance',enter('balance'))
